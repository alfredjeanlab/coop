...
use coop::{FALLBACK_TERM_COLS, FALLBACK_TERM_ROWS};
...
async fn session_input_roundtrip() -> anyhow::Result<()> {
    ...
    let backend =
        NativePty::spawn(&["/bin/cat".into()], FALLBACK_TERM_COLS, FALLBACK_TERM_ROWS, &[])?;
    let session = Session::new(
        &config,
        SessionConfig::new(Arc::clone(&app_state), backend, consumer_input_rx),
    );
...
async fn session_exited_state_broadcast() -> anyhow::Result<()> {
    ...
    let backend = NativePty::spawn(&["true".into()], FALLBACK_TERM_COLS, FALLBACK_TERM_ROWS, &[])?;
    let session = Session::new(
        &config,
        SessionConfig::new(Arc::clone(&app_state), backend, consumer_input_rx),
    );
...
async fn http_health_endpoint() -> anyhow::Result<()> {
    ...
    assert_eq!(health.terminal.cols, FALLBACK_TERM_COLS);
    assert_eq!(health.terminal.rows, FALLBACK_TERM_ROWS);
    Ok(())
}
...
async fn http_screen_endpoint() -> anyhow::Result<()> {
    ...
    assert_eq!(screen.cols, FALLBACK_TERM_COLS);
    assert_eq!(screen.rows, FALLBACK_TERM_ROWS);
