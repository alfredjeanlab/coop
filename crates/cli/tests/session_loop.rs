...
async fn session_echo_captures_output_and_exits_zero() -> anyhow::Result<()> {
    ...
    let backend =
        NativePty::spawn(&["echo".into(), "integration".into()], TEST_COLS, TEST_ROWS, &[])?;
...
async fn session_exited_state_broadcast() -> anyhow::Result<()> {
    ...
    let backend = NativePty::spawn(&["true".into()], TEST_COLS, TEST_ROWS, &[])?;
    let session = Session::new(
        &config,
        SessionConfig::new(Arc::clone(&app_state), backend, consumer_input_rx),
    );
...
async fn http_health_endpoint() -> anyhow::Result<()> {
    ...
    assert_eq!(health.terminal.cols, TEST_COLS);
    assert_eq!(health.terminal.rows, TEST_ROWS);
    Ok(())
}
...
async fn http_screen_endpoint() -> anyhow::Result<()> {
    ...
    assert_eq!(screen.cols, TEST_COLS);
    assert_eq!(screen.rows, TEST_ROWS);
...
async fn full_stack_echo_screen_via_http() -> anyhow::Result<()> {
    ...
    let backend =
        NativePty::spawn(&["echo".into(), "fullstack".into()], TEST_COLS, TEST_ROWS, &[])?;
    let session = Session::new(
        &config,
        SessionConfig::new(Arc::clone(&app_state), backend, consumer_input_rx),
    );
