...
use coop::test_support::{AppStateBuilder, TEST_RING_SIZE, TEST_TERMINAL_COLS, TEST_TERMINAL_ROWS};
...
async fn session_exited_state_broadcast() -> anyhow::Result<()> {
    ...
    let backend = NativePty::spawn(&["true".into()], TEST_TERMINAL_COLS, TEST_TERMINAL_ROWS, &[])?;
    let session = Session::new(
        &config,
        SessionConfig::new(Arc::clone(&app_state), backend, consumer_input_rx),
    );
...
async fn http_health_endpoint() -> anyhow::Result<()> {
    ...
    assert_eq!(health.terminal.cols, TEST_TERMINAL_COLS);
    assert_eq!(health.terminal.rows, TEST_TERMINAL_ROWS);
    Ok(())
}
...
async fn http_screen_endpoint() -> anyhow::Result<()> {
    ...
    assert_eq!(screen.cols, TEST_TERMINAL_COLS);
    assert_eq!(screen.rows, TEST_TERMINAL_ROWS);
