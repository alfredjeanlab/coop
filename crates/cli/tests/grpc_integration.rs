...
use coop::test_support::{
    spawn_grpc_server, AppStateBuilder, StubNudgeEncoder, TEST_COLS, TEST_RING_SIZE, TEST_ROWS,
};
...
async fn grpc_get_screen() -> anyhow::Result<()> {
    ...
    assert_eq!(resp.cols, i32::from(TEST_COLS));
    assert_eq!(resp.rows, i32::from(TEST_ROWS));
...
async fn grpc_stream_screen() -> anyhow::Result<()> {
    ...
    assert_eq!(snap.cols, i32::from(TEST_COLS));
    assert_eq!(snap.rows, i32::from(TEST_ROWS));
