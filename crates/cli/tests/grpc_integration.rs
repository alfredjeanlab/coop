...
async fn grpc_get_screen() -> anyhow::Result<()> {
    ...
    assert_eq!(resp.cols, FALLBACK_TERM_COLS as i32);
    assert_eq!(resp.rows, FALLBACK_TERM_ROWS as i32);
...
async fn grpc_stream_screen() -> anyhow::Result<()> {
    ...
    assert_eq!(snap.cols, FALLBACK_TERM_COLS as i32);
    assert_eq!(snap.rows, FALLBACK_TERM_ROWS as i32);
