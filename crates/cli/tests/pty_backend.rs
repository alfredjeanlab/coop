...
use coop::test_support::{TEST_COLS, TEST_ROWS};
use tokio::sync::mpsc;
...
async fn spawn_and_capture() {
    ...
    let mut pty = NativePty::spawn(&["echo".into(), "hello".into()], TEST_COLS, TEST_ROWS, &[])
        .expect("spawn failed");
...
async fn input_delivery() {
    ...
    let mut pty =
        NativePty::spawn(&["/bin/cat".into()], TEST_COLS, TEST_ROWS, &[]).expect("spawn failed");
...
async fn screen_integration() {
    ...
    let mut screen = Screen::new(TEST_COLS, TEST_ROWS);
    while let Ok(chunk) = output_rx.try_recv() {
        screen.feed(&chunk);
    }
