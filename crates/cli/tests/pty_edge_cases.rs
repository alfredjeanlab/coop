...
async fn child_exit_produces_eof() -> anyhow::Result<()> {
    ...
    let mut pty =
        NativePty::spawn(&["true".into()], DEFAULT_TERMINAL_COLS, DEFAULT_TERMINAL_ROWS, &[])?;
...
async fn resize_reflected_in_stty() -> anyhow::Result<()> {
    ...
    let mut pty =
        NativePty::spawn(&["/bin/sh".into()], DEFAULT_TERMINAL_COLS, DEFAULT_TERMINAL_ROWS, &[])?;
...
async fn rapid_input_output() -> anyhow::Result<()> {
    ...
    let mut pty =
        NativePty::spawn(&["/bin/cat".into()], DEFAULT_TERMINAL_COLS, DEFAULT_TERMINAL_ROWS, &[])?;
    let handle = tokio::spawn(async move { pty.run(output_tx, input_rx, resize_rx).await });
