...
use coop::run::{EVENT_CHANNEL_CAPACITY, IO_CHANNEL_CAPACITY};
use coop::screen;
...
async fn child_exit_produces_eof() -> anyhow::Result<()> {
    ...
    let mut pty =
        NativePty::spawn(&["true".into()], screen::DEFAULT_COLS, screen::DEFAULT_ROWS, &[])?;
...
async fn child_killed_produces_signal() -> anyhow::Result<()> {
    let (output_tx, _output_rx) = mpsc::channel(EVENT_CHANNEL_CAPACITY);
    let (_input_tx, input_rx) = mpsc::channel::<BackendInput>(64);
    let (_resize_tx, resize_rx) = mpsc::channel(4);
...
async fn resize_reflected_in_stty() -> anyhow::Result<()> {
    ...
    let mut pty =
        NativePty::spawn(&["/bin/sh".into()], screen::DEFAULT_COLS, screen::DEFAULT_ROWS, &[])?;
...
async fn large_output_through_session() -> anyhow::Result<()> {
    ...
    let (input_tx, consumer_input_rx) = mpsc::channel(EVENT_CHANNEL_CAPACITY);
