...
async fn child_exit_produces_eof() -> anyhow::Result<()> {
    let (output_tx, mut output_rx) = mpsc::channel(coop::EVENT_CHANNEL_CAPACITY);
    let (_input_tx, input_rx) = mpsc::channel::<BackendInput>(coop::EVENT_CHANNEL_CAPACITY);
    let (_resize_tx, resize_rx) = mpsc::channel(4);

    let mut pty =
        NativePty::spawn(&["true".into()], coop::DEFAULT_TERM_COLS, coop::DEFAULT_TERM_ROWS, &[])?;
...
async fn child_killed_produces_signal() -> anyhow::Result<()> {
    let (output_tx, _output_rx) = mpsc::channel(coop::EVENT_CHANNEL_CAPACITY);
    let (_input_tx, input_rx) = mpsc::channel::<BackendInput>(coop::EVENT_CHANNEL_CAPACITY);
    let (_resize_tx, resize_rx) = mpsc::channel(4);

    let mut pty = NativePty::spawn(
        &["/bin/sleep".into(), "60".into()],
        coop::DEFAULT_TERM_COLS,
        coop::DEFAULT_TERM_ROWS,
        &[],
    )?;
...
async fn resize_reflected_in_stty() -> anyhow::Result<()> {
    let (output_tx, mut output_rx) = mpsc::channel(coop::EVENT_CHANNEL_CAPACITY);
    let (input_tx, input_rx) = mpsc::channel::<BackendInput>(coop::EVENT_CHANNEL_CAPACITY);
...
async fn large_output_through_session() -> anyhow::Result<()> {
    ...
    let (input_tx, consumer_input_rx) = mpsc::channel(coop::EVENT_CHANNEL_CAPACITY);
    let app_state = AppStateBuilder::new()
        .ring_size(coop::DEFAULT_RING_SIZE) // 1MB
...
async fn binary_output_no_panic() -> anyhow::Result<()> {
    let (output_tx, mut output_rx) = mpsc::channel(coop::DATA_CHANNEL_CAPACITY);
    let (_input_tx, input_rx) = mpsc::channel::<BackendInput>(coop::EVENT_CHANNEL_CAPACITY);
    let (_resize_tx, resize_rx) = mpsc::channel(4);
...
async fn rapid_input_output() -> anyhow::Result<()> {
    let (output_tx, mut output_rx) = mpsc::channel(coop::DATA_CHANNEL_CAPACITY);
    let (input_tx, input_rx) = mpsc::channel::<BackendInput>(coop::DATA_CHANNEL_CAPACITY);
    let (_resize_tx, resize_rx) = mpsc::channel(4);
...
async fn signal_delivery_sigint() -> anyhow::Result<()> {
    ...
    let (input_tx, consumer_input_rx) = mpsc::channel(coop::EVENT_CHANNEL_CAPACITY);
    let app_state =
        AppStateBuilder::new().ring_size(coop::TEST_RING_SIZE).build_with_sender(input_tx.clone());
