...
use coop::test_support::{
    AppStateBuilder, TEST_LARGE_RING_SIZE, TEST_RING_SIZE, TEST_TERMINAL_COLS, TEST_TERMINAL_ROWS,
};
...
async fn child_exit_produces_eof() -> anyhow::Result<()> {
    ...
    let mut pty = NativePty::spawn(&["true".into()], TEST_TERMINAL_COLS, TEST_TERMINAL_ROWS, &[])?;
...
async fn resize_reflected_in_stty() -> anyhow::Result<()> {
    ...
    let mut pty =
        NativePty::spawn(&["/bin/sh".into()], TEST_TERMINAL_COLS, TEST_TERMINAL_ROWS, &[])?;
...
async fn large_output_through_session() -> anyhow::Result<()> {
    ...
    let app_state =
        AppStateBuilder::new().ring_size(TEST_LARGE_RING_SIZE).build_with_sender(input_tx);
