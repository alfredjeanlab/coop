...
#[test]
fn feed_plain_text() {
    let mut screen = Screen::new(crate::DEFAULT_TERM_COLS, crate::DEFAULT_TERM_ROWS);
...
fn alt_screen_toggle_split_across_chunks() {
    let screen = Screen::new(crate::DEFAULT_TERM_COLS, crate::DEFAULT_TERM_ROWS);
    assert!(!screen.is_alt_screen());

    // Split "\x1b[?1049h" across two feed() calls at every possible boundary.
    let seq = b"\x1b[?1049h";
    for split in 1..seq.len() {
        let mut s = Screen::new(crate::DEFAULT_TERM_COLS, crate::DEFAULT_TERM_ROWS);
        s.feed(&seq[..split]);
        s.feed(&seq[split..]);
