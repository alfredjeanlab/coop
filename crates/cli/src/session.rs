...
/// Channel capacity for backend I/O streams (input and output).
const BACKEND_IO_CHANNEL_CAPACITY: usize = 256;
/// Channel capacity for terminal resize events.
const RESIZE_CHANNEL_CAPACITY: usize = 4;
/// Channel capacity for detector state updates.
...
impl Session {
    ...
    pub fn new(config: &Config, session: SessionConfig) -> Self {
        ...
        let (backend_output_tx, backend_output_rx) = mpsc::channel(BACKEND_IO_CHANNEL_CAPACITY);
        let (backend_input_tx, backend_input_rx) =
            mpsc::channel::<BackendInput>(BACKEND_IO_CHANNEL_CAPACITY);
