...
use crate::run::{DATA_CHANNEL_CAPACITY, SIGNAL_CHANNEL_CAPACITY};
...
impl Session {
    ...
    pub fn new(config: &Config, session: SessionConfig) -> Self {
        ...
        let (backend_output_tx, backend_output_rx) = mpsc::channel(DATA_CHANNEL_CAPACITY);
        let (backend_input_tx, backend_input_rx) =
            mpsc::channel::<BackendInput>(DATA_CHANNEL_CAPACITY);
        ...
        let (detector_tx, detector_rx) = mpsc::channel(SIGNAL_CHANNEL_CAPACITY);
...
/// Maximum number of screen-scraping attempts when enriching prompt options.
const PROMPT_ENRICH_MAX_ATTEMPTS: u32 = 10;
/// Polling interval between prompt enrichment attempts.
...
/// Retries up to `PROMPT_ENRICH_MAX_ATTEMPTS` times, then falls back to universal Accept/Cancel options that encode to Enter/Esc.
