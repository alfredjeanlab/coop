...
/// Agent settings containing hooks, permissions, environment variables.
///
/// Used for both orchestrator base settings (from the agent config file) and
/// coop's generated hook configuration. [`merge_settings`] combines both layers.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct AgentSettings {
    /// Hook definitions keyed by hook type (e.g. `"SessionStart"`, `"PostToolUse"`).
    /// Each value is an array of hook matchers; during merge, arrays are concatenated.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub hooks: Option<BTreeMap<String, Vec<serde_json::Value>>>,
    ...
    /// Catch-all for other top-level keys (plugins, etc.) that pass through unchanged.
    #[serde(flatten)]
    pub extra: BTreeMap<String, serde_json::Value>,
}

/// Permission rules for agent tool access.
#[derive(Debug, Clone, Default, Serialize, Deserialize)]
pub struct Permissions {
    /// Allowed tool patterns (e.g. `"Bash"`, `"Bash(coop send:*)"`)
    #[serde(default)]
...
/// MCP server definitions map (`"server-name"` → server config).
pub type McpConfig = BTreeMap<String, McpServer>;
...
pub struct McpServer {
    ...
    pub args: Vec<String>,
    /// Catch-all for other server config fields (env, url, transport, etc.).
    #[serde(flatten)]
    pub extra: BTreeMap<String, serde_json::Value>,
}
...
pub fn merge_settings(orchestrator: &AgentSettings, coop: AgentSettings) -> AgentSettings {
    ...
    if let Some(coop_hooks) = coop.hooks {
        let merged_hooks = merged.hooks.get_or_insert_with(BTreeMap::new);
        for (hook_type, coop_entries) in coop_hooks {
            merged_hooks.entry(hook_type).or_default().extend(coop_entries);
        }
    }
