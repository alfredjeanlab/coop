...
fn generated_config_has_required_hooks() {
    ...
    assert!(config.hooks.contains_key("BeforeAgent"));
    assert!(config.hooks.contains_key("BeforeTool"));
    assert!(config.hooks.contains_key("AfterTool"));
    assert!(config.hooks.contains_key("AfterAgent"));
    ...
    for hook_name in [
        ...
        let groups = &config.hooks[hook_name];
        assert!(!groups.is_empty(), "{hook_name} should have at least one group");
        assert_eq!(groups[0].matcher, "*", "{hook_name} matcher should be wildcard");
        assert!(!groups[0].hooks.is_empty(), "{hook_name} should have at least one hook");
        assert_eq!(groups[0].hooks[0].hook_type, "command", "{hook_name} type should be command");
    }
}
