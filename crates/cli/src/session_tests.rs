...
async fn echo_exits_with_zero() -> anyhow::Result<()> {
    ...
    let (input_tx, consumer_input_rx) = mpsc::channel(crate::run::SIGNAL_CHANNEL_CAPACITY);
    let app_state = AppStateBuilder::new().ring_size(TEST_RING_SIZE).build_with_sender(input_tx);

    let backend =
        NativePty::spawn(&["echo".into(), "hello".into()], DEFAULT_COLS, DEFAULT_ROWS, &[])?;
...
async fn output_captured_in_ring_and_screen() -> anyhow::Result<()> {
    ...
    let backend =
        NativePty::spawn(&["echo".into(), "hello-ring".into()], DEFAULT_COLS, DEFAULT_ROWS, &[])?;
