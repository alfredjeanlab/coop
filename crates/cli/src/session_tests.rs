...
use crate::test_support::{
    AppStateBuilder, MockDetector, MockPty, StubRespondEncoder, TEST_COLS, TEST_RING_SIZE,
    TEST_ROWS,
};
...
async fn echo_exits_with_zero() -> anyhow::Result<()> {
    ...
    let backend = NativePty::spawn(&["echo".into(), "hello".into()], TEST_COLS, TEST_ROWS, &[])?;
...
async fn output_captured_in_ring_and_screen() -> anyhow::Result<()> {
    ...
    let backend =
        NativePty::spawn(&["echo".into(), "hello-ring".into()], TEST_COLS, TEST_ROWS, &[])?;
