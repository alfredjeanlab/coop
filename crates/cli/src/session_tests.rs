...
use crate::run::NOTIFY_CHANNEL_CAPACITY;
use crate::session::{Session, SessionConfig};
use crate::test_support::{
    AppStateBuilder, MockDetector, MockPty, StubRespondEncoder, TEST_RING_SIZE, TEST_TERMINAL_COLS,
    TEST_TERMINAL_ROWS,
};
...
async fn echo_exits_with_zero() -> anyhow::Result<()> {
    ...
    let (input_tx, consumer_input_rx) = mpsc::channel(NOTIFY_CHANNEL_CAPACITY);
