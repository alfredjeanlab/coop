...
/// Standard ring buffer size for tests (64 KiB).
...
impl AppStateBuilder {
    ...
    pub fn build_with_sender(self, input_tx: mpsc::Sender<InputEvent>) -> Arc<Store> {
        let (output_tx, _) = broadcast::channel::<OutputEvent>(crate::DATA_CHANNEL_CAPACITY);
        let (state_tx, _) = broadcast::channel::<TransitionEvent>(crate::SIGNAL_CHANNEL_CAPACITY);
        let (prompt_tx, _) = broadcast::channel::<PromptOutcome>(crate::SIGNAL_CHANNEL_CAPACITY);
