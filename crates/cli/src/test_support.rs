...
impl AppStateBuilder {
    ...
    pub fn build_with_sender(self, input_tx: mpsc::Sender<InputEvent>) -> Arc<Store> {
        ...
        let (state_tx, _) =
            broadcast::channel::<TransitionEvent>(crate::run::STATE_CHANNEL_CAPACITY);
        let (prompt_tx, _) =
            broadcast::channel::<PromptOutcome>(crate::run::PROMPT_CHANNEL_CAPACITY);

        Arc::new(Store {
            terminal: Arc::new(TerminalState {
                screen: RwLock::new(Screen::new(
                    crate::screen::FALLBACK_COLS,
                    crate::screen::FALLBACK_ROWS,
                )),
