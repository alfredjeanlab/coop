...
/// Ring buffer size used in tests (64 KiB).
...
impl AppStateBuilder {
    ...
    pub fn build_with_sender(self, input_tx: mpsc::Sender<InputEvent>) -> Arc<AppState> {
        let (output_tx, _) =
            broadcast::channel::<OutputEvent>(crate::run::OUTPUT_BROADCAST_CAPACITY);
        let (state_tx, _) =
            broadcast::channel::<StateChangeEvent>(crate::run::STATE_BROADCAST_CAPACITY);
        let (prompt_tx, _) =
            broadcast::channel::<PromptEvent>(crate::run::PROMPT_BROADCAST_CAPACITY);
