...
use crate::test_support::{TEST_RING_SIZE, TEST_TERMINAL_COLS, TEST_TERMINAL_ROWS};
...
async fn run_statusline_cmd_captures_output() {
    let state = AttachState::new(TEST_TERMINAL_COLS, TEST_TERMINAL_ROWS);
...
async fn run_statusline_cmd_expands_state() {
    let mut state = AttachState::new(TEST_TERMINAL_COLS, TEST_TERMINAL_ROWS);
