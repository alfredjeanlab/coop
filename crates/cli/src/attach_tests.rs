...
use crate::{DEFAULT_TERM_COLS, DEFAULT_TERM_ROWS, EVENT_CHANNEL_CAPACITY, TEST_RING_SIZE};
...
async fn run_statusline_cmd_captures_output() {
    let state = AttachState::new(DEFAULT_TERM_COLS, DEFAULT_TERM_ROWS);
...
async fn run_statusline_cmd_expands_state() {
    let mut state = AttachState::new(DEFAULT_TERM_COLS, DEFAULT_TERM_ROWS);
