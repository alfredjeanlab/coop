...
fn builtin_statusline_uptime_increases() {
    let state = AttachState {
        ...
        cols: FALLBACK_TERM_COLS,
        rows: FALLBACK_TERM_ROWS,
...
async fn run_statusline_cmd_captures_output() {
    let state = AttachState::new(FALLBACK_TERM_COLS, FALLBACK_TERM_ROWS);
...
async fn run_statusline_cmd_expands_state() {
    let mut state = AttachState::new(FALLBACK_TERM_COLS, FALLBACK_TERM_ROWS);
