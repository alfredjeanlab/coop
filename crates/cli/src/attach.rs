...
async fn attach(
    ...
    let (init_cols, init_rows) =
        terminal_size().unwrap_or((crate::screen::DEFAULT_COLS, crate::screen::DEFAULT_ROWS));
    ...
    let (stdin_tx, mut stdin_rx) = mpsc::channel::<Vec<u8>>(crate::run::CONTROL_CHANNEL_CAPACITY);
