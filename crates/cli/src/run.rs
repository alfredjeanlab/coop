...
/// Channel capacity for input and output I/O streams.
pub(crate) const IO_CHANNEL_CAPACITY: usize = 256;
/// Channel capacity for state and prompt notification broadcasts.
pub(crate) const NOTIFY_CHANNEL_CAPACITY: usize = 64;
...
pub async fn prepare(config: Config) -> anyhow::Result<PreparedSession> {
    ...
    let (state_tx, _) = broadcast::channel(NOTIFY_CHANNEL_CAPACITY);
    let (prompt_tx, _) = broadcast::channel(NOTIFY_CHANNEL_CAPACITY);
