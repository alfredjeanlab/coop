...
/// Channel capacity for high-throughput data channels (input, output, backend I/O).
pub(crate) const DATA_CHANNEL_CAPACITY: usize = 256;
/// Channel capacity for low-throughput signal channels (state transitions, prompts, detection).
...
pub async fn prepare(config: Config) -> anyhow::Result<PreparedSession> {
    ...
    let (input_tx, consumer_input_rx) = mpsc::channel(DATA_CHANNEL_CAPACITY);
    let (output_tx, _) = broadcast::channel(DATA_CHANNEL_CAPACITY);
    let (state_tx, _) = broadcast::channel(SIGNAL_CHANNEL_CAPACITY);
    let (prompt_tx, _) = broadcast::channel(SIGNAL_CHANNEL_CAPACITY);
