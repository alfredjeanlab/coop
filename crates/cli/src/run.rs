...
pub async fn prepare(config: Config) -> anyhow::Result<PreparedSession> {
    ...
    let (input_tx, consumer_input_rx) = mpsc::channel(CHANNEL_CAP_IO);
    let (output_tx, _) = broadcast::channel(CHANNEL_CAP_IO);
    let (state_tx, _) = broadcast::channel(CHANNEL_CAP_SIGNAL);
    let (prompt_tx, _) = broadcast::channel(CHANNEL_CAP_SIGNAL);
